...
public class P12Utils {
    ...
    /**
     * Extrae de un archivo p12 los siguientes elementos:
     * Private key
     * Certificado
     * X509SerialNumber
     * X509IssuerName
     ...
     * @param certPath Direcci�n en disco donde se encuentra el archivo p12, si esta es null se usa
     *                 la variable de entorno CERT_PATH
     ...
     */
    public static CertificateInfo dataFromP12(String p12Path, String password) throws P12Exception {
       
        String pemPath = FacturaElectronicaUtils.normalize("/tmp/pem.pem");
        String cerPath = FacturaElectronicaUtils.normalize("/tmp/cer.cer");
        try {
            String openssl = "openssl pkcs12 -in " + p12Path + " -passin pass:"
                    + password + " -out " + pemPath + "  -nodes";
