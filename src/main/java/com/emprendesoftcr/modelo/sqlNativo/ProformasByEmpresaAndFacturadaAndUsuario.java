...
@BaseNativeQuery(name = "proformas_fact_9", query = "SELECT facturas.id,facturas.usuario_id,facturas.fecha_emision,facturas.cliente_id,facturas.nombre_factura,facturas.total_impuesto,facturas.total_descuentos,facturas.total_comprobante,usuarios.nombre_usuario,clientes.nombre_completo,facturas.empresa_id,facturas.consecutivo_proforma,facturas.estado,facturas.tipo_doc,facturas.numero_consecutivo FROM facturas" + " left join clientes on clientes.id = facturas.cliente_id " + " left JOIN usuarios on usuarios.id = facturas.usuario_id " + " left join empresas on empresas.id = facturas.empresa_id " + " where facturas.empresa_id = :ID_EMPRESA and facturas.estado =2  and facturas.usuario_id = :IDUSUARIO and facturas.consecutivo_proforma !=''" + " order by facturas.fecha_emision desc ")
@Entity
public class ProformasByEmpresaAndFacturadaAndUsuario implements Serializable {
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		ProformasByEmpresaAndFacturadaAndUsuario other = (ProformasByEmpresaAndFacturadaAndUsuario) obj;
		if (cliente == null) {
