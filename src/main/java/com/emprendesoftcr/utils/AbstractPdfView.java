/*
 * Se hace una copia de la clase AbstractPdfView ya que no se necesita grabar un archivo.
...
/**
 * Abstract superclass for PDF views, using Bruno Lowagie's <a href="http://www.lowagie.com/iText">iText</a> package. Application-specific view classes will extend this class. The view will be held in
 * the subclass itself, not in a template.
 * <p>
...
public abstract class AbstractPdfView extends AbstractView {
	...
	protected final void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		/*
		 * ByteArrayOutputStream baos = createTemporaryOutputStream(); // Apply preferences and build metadata. Document document = newDocument(); PdfWriter writer = newWriter(document, baos);
		 * prepareWriter(model, writer, request); buildPdfMetadata(model, document, request); // Build PDF document. //document.open();
		 */
		buildPdfDocument(model, request, response);
		// document.close();
	...
	/**
	 * Prepare the given PdfWriter. Called before building the PDF document, that is, before the call to <code>Document.open()</code>.
	 * <p>
	 * Useful for registering a page event listener, for example. The default implementation sets the viewer preferences as returned by this class's <code>getViewerPreferences()</code> method.
	 ...
	 * <p>
	 * By default returns <code>AllowPrinting</code> and <code>PageLayoutSinglePage</code>, but can be subclassed. The subclass can either have fixed preferences or retrieve them from bean properties
	 * defined on the View.
	 ...
	 * Default is an empty implementation. Subclasses may override this method to add meta fields such as title, subject, author, creator, keywords, etc. This method is called after assigning a
	 * PdfWriter to the Document and before calling <code>document.open()</code>.
	 * @param model the model, in case meta information must be populated from it
	...
	/**
	 * Subclasses must implement this method to build an iText PDF document, given the model. Called between <code>Document.open()</code> and <code>Document.close()</code> calls.
	 * <p>
	 ...
	 */
	protected abstract void buildPdfDocument(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception;
