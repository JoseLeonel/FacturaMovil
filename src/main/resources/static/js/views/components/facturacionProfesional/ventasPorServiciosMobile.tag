<ventasPorServiciosMobile>

  <main class="flex cardMobile" show ={mostrarProductos ==2}>
    <div >
        <div class="cardMobile-titulo-card"><span>Crear factura electronica</span></div>
        <div class="cardMobile-titulo"><span>Digite el detalle de la factura</span></div>
        <div><textarea id="descripcion" name="descripcion" rows="4" cols="50"></textarea></div>
        <div class="cardMobile-titulo"><span>{$.i18n.prop("factura.tipoDocumento")}</span></div>  
        <div>
            <select  onchange= {__formaReferencias} class=" tipoDoc" id="tipoDoc" name="tipoDoc" >
                <option each={comboTipoDocumentos}  value="{estado}" selected="{factura.tipoDoc ==estado?true:false}" >{descripcion}</option>
            </select>
        </div>
        <div class="cardMobile-titulo"> <span>{$.i18n.prop("factura.tipo.moneda")}</span> </div>  
        <div>    
            <select class="codigoMoneda" id="codigoMoneda" name="codigoMoneda" >
   ...
   <div class = "item-principal-categorias" >
      <div each ={categorias.data}  onclick={__ArticulosXCategorias}>
         <span>{descripcion}</span>
      </div>
   </div> 


    <div  show ={mostrarProductos} id="listaArticulos">
        <div  class="cardArticulo" each={articulos.data}  >
            <div  class="cardArticulo-item">
                <div class="cardArticulo-item-imagen" >
                ...
                <div class="cardArticulo-item-descripcion flex-item-descripcion">    

                    <div class="flex-item-descripcion-item"> <span >{descripcion.length > 50 ? descripcion.substring(0,50) +"...":descripcion}</span></div>
                    <div class="flex-item-descripcion-item flex-item-descripcion-item-precio"> <span >{"Precio: " + precioPublicoformato } </span></div>
                </div> 
                <div class = "componente-botones-totales">
                    <div class="cardArticulo-item-button">
                        <div><button onclick={__sumarItem} class = "btn-mas boton-redonde" type="button" ><i class="glyphicon glyphicon-plus"></i></button></div>
                        <div><button type="button"   class="btn-total " >{cantidaProducto}</button></div>
                        <div><button type="button"   onclick={__restarItem} class="btn-minus boton-redonde" ><i class="glyphicon glyphicon-minus"></i></button></div>
                    </div>   
                    <div class="">
                        <div></div>  
                        <div class="item-total-general">  
                             <span>{montoTotalLinea}</span> 
                        </div>   
                        <div></div>    
                    </div>
                </div>
    ...
    <div class="footer-principal">
            <div><span>Item:{numeroLinea}</span></div>
            <div class="footer-ver-detalle "><button class="footer-ver-detalle button">Ver Detalle</button></div>
            <div ><span >Total : {totalComprobante}</span></div>
    </div>

<!-- Primer Paso -->
<div show={formularioDetalle == 1}>
    <div class="encabezado">
        <div><span>T.C: $ {tipoCambio.total} Banco Central</span></div>
        <div>
             <button onclick ={__regresarProductos}  type="button" >Total 2500</button>
        </div>
            ...
            <div class="box box-solid box-primary">
                <div class="box-body">
                    <form id = "formulario" name ="formulario"   class="advanced-search-form formulario">
                        <div class="formatoMonto">
                            
                            <div class="formatoTitulos">
                                <span>{$.i18n.prop("factura.tipoDocumento")}</span> 
                            </div>    
                            <div>
                                <select  onchange= {__formaReferencias} class="selectFormato formatoTitulos tipoDoc" id="tipoDoc" name="tipoDoc" >
                                    <option each={comboTipoDocumentos}  value="{estado}" selected="{factura.tipoDoc ==estado?true:false}" >{descripcion}</option>
                                </select>
                            </div>
                        </div>
                        <div class="formatoMonto">
                            <div class="formatoTitulos">
                                <span>{$.i18n.prop("factura.detalle.descripcion.producto")} <span class="requeridoDato">*</span></span> 
                            </div>    
                            <div>    
                                <textarea class = "formatoTextArea descripcionProducto" name="descripcionProducto"  id="descripcionProducto" rows="5" cols="50" >"{articulo.descripcion}"</textarea>
                            </div>
                        </div>
                        <div class="formatoMonto">
                            <div class="formatoTitulos">
                                <span>{$.i18n.prop("factura.tipo.moneda")}</span> 
                            </div>
                            <div>    
                                <select class="selectFormato formatoTitulos codigoMoneda" id="codigoMoneda" name="codigoMoneda" >
                                    <option each={monedas}  value="{estado}" selected="{factura.codigoMoneda ==estado?true:false}" >{descripcion}</option>
                                </select>
                            </div>
                        </div>
                        <div class="formatoMonto">
                            <div class="formatoTitulos">
                                <span>{$.i18n.prop("factura.detalle.total.producto")} </span> 
                            </div> 
                            <div class="formatoTitulos">   
                                <input type="number" step="any" class=" formatoMontoPrecio precio" id="precio"  value = "{articulo.precioPublico}">
                            </div>
                        </div>
                    </form>    
                </div>
                <div class="box-footer">
                    <div class="row">
                       <div class="col-md-6 col-sx-12 col-sm-6 col-lg-6">
                            <button onclick ={__regresarProductos}  type="button" class="btn-regresar btn-back pull-left"  id= "btnCancelarEmpresa" name = "btnCancelarEmpresa">
                                {$.i18n.prop("btn.volver")}
                            </button>
                       </div>
                       <div class="col-md-6 col-sx-12 col-sm-6 col-lg-6">
                            <button  onclick={_PrimerPaso}  class="btn-factura btn-siguiente pull-right " >  {$.i18n.prop("btn.siguiente")}</button>
                       </div>
                    </div>   
                </div>
            </div>   
...
__sumarItem(e){
   __buscarArticuloSumarOrRestar(e.item,1)
   self.articulo = e.item 
   __agregarArticulo(1); 
   _asignarDetalleFactura(self.articulos.data);
   __calculate()
}

__restarItem(e){
   __buscarArticuloSumarOrRestar(e.item,2)
   self.articulo = e.item 
   _asignarDetalleFactura(self.articulos.data);
   __calculate()
...
function _asignarDetalleFactura(articuloSeleccionados){
  self.detail = []
  self.numeroLinea = 0;
  self.update()
  for (var count = 0; count < articuloSeleccionados.length; count++) {
       var cantidaProducto = __valorNumerico(articuloSeleccionados[count].cantidaProducto)
       if(cantidaProducto > 0){
           var itemNuevo = setItemNuevo(articuloSeleccionados[count],self.numeroLinea)          
           self.numeroLinea ++;
...
function setItemNuevo(data,numeroLinea){
    
    var resultaMontoImpuesto = __valorNumerico(data.impuesto)
    var precioUnitario  = getPrecioUnitario(data.precioPublico,resultaMontoImpuesto)
    var montoTotal      = getMontoTotal(precioUnitario,data.cantidaProducto)
    var montoDescuento  = 0
    var naturalezaDescuento = ""
    var subTotal        = montoTotal
    var montoImpuesto1  = 0
    var montoImpuesto   = _calcularImpuesto(subTotal,__valorNumerico(data.impuesto) ==null?0:__valorNumerico(data.impuesto))
    ...
    var item = {
       ...
       pesoPrioridad   : 0,
       tipoImpuesto    : data.tipoImpuesto ==null?"":data.tipoImpuesto,
       tipoImpuesto1   : "",
       tipoCodigo      : data.tipoCodigo,
       unidadMedida    : data.unidadMedida,
       iva             : __valorNumerico(data.impuesto),
       iva1            : 0,
       codigo          : data.codigo,
       descripcion     : data.descripcion,
       cantidad        : data.cantidaProducto,
       precioUnitario  : __valorNumerico(precioUnitario),
       impuesto        : __valorNumerico(data.impuesto),
       impuesto1        : 0,
       ...
       pesoTransporte :  __valorNumerico(data.pesoTransporte),
       pesoTransporteTotal :__valorNumerico(data.pesoTransporte),
       montoExoneracion:0,
       montoExoneracion1:0,
...
/**
data = articulo operacion = 1= suma 2= resta
**/
function __buscarArticuloSumarOrRestar(data,operacion){
    if(self.articulos.data[0] == null){
        data.cantidaProducto = _SumaRestaCantidad(data,operacion)
        data.montoTotalLinea = __multiplicarCantidadPorPrecio(data)
        self.articulos.data[0] = data;

    }else{
       for (var count = 0; count < self.articulos.data.length; count++) {
            if (self.articulos.data[count].codigo == data.codigo ){
                self.item          = self.articulos.data[count];
                self.item.cantidaProducto = _SumaRestaCantidad(self.item,operacion)
                self.item.montoTotalLinea = __multiplicarCantidadPorPrecio(self.item)
                self.articulos.data[count] = self.item;
            }
        }
    }   
    self.update(); 
}
function __multiplicarCantidadPorPrecio(data){
    return data.precioPublico *  data.cantidaProducto
}
function _SumaRestaCantidad(data,operacion){
  return operacion == 1 ?data.cantidaProducto + 1:data.cantidaProducto > 0 ? data.cantidaProducto - 1:0;
...
function __ListaArticulosXCategorias(){
    ...
    $.ajax({
        ...
        datatype: "json",
        data: {categoria:self.categoria.id,estado:'Activo'},
        method:"POST",
        success: function (result) {
            if(result.aaData.length > 0){
