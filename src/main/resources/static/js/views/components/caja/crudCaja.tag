<caja-crud>
    ...
    <div  class="row titulo-encabezado"  >
        <div  class="col-xs-8 col-sm-8 col-md-8 col-lg-8">
            <h1 ><i class="fa fa-cog"></i>&nbsp {$.i18n.prop("caja.titulo")}  </h1>
        </div>
        ...
        <div class="row">
            <div class="col-sx-12  col-lg-12  col-md-12 col-sm-12 " style="width:98.50%;">
                    <table id="tableListar" class="display table responsive table-hover nowrap table-condensed tableListar"   cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th class="table-header" >{$.i18n.prop("caja.descripcion")}   </th>
                                <th class="table-header" >{$.i18n.prop("caja.terminal")}      </th>
                                <th class="table-header" >{$.i18n.prop("caja.created_at")}    </th>
                                <th class="table-header" >{$.i18n.prop("caja.updated_at")}    </th>
                                <th class="table-header" >{$.i18n.prop("caja.usuario")}       </th>
                                <th class="table-header" >{$.i18n.prop("caja.estado")}        </th>
                                <th class="table-header" >{$.i18n.prop("listado.acciones")}   </th>
                            </tr>
                        </thead>
                        <tfoot style="display: table-header-group;">
                            <tr>
                                <th>{$.i18n.prop("caja.descripcion")}   </th>
                                <th>{$.i18n.prop("caja.terminal")}      </th>
                                <th>{$.i18n.prop("caja.created_at")}    </th>
                                <th>{$.i18n.prop("caja.updated_at")}    </th>
                                <th>{$.i18n.prop("caja.usuario")}       </th>
                                <th>{$.i18n.prop("caja.estado")}        </th>
                                <th>                                    </th>
                            </tr>
                        </tfoot>
                    </table>
        ...
        <div class="col-md-8 col-lg-8 col-sx-12 col-sm-8">
            <div class="box box-solid box-primary">
                <div class="box-header with-border">
                    <h1 class="box-title"><i class="fa fa-edit"></i>&nbsp {caja.id > 0 ? $.i18n.prop("caja.modificar")   :$.i18n.prop("caja.agregar")}     </h1>
                </div>
                <div class="box-body">
                    <form id = "formulario" name ="formulario "   class="advanced-search-form">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-sx-12 col-sm-6 col-lg-6">
                                <label class="knob-label">{$.i18n.prop("caja.estado")}</label>
                                <select  class="form-control" id="estado" name="estado" >
                                    <option  each={estados}  value="{codigo}" selected="{caja.estado ==codigo?true:false}" >{descripcion}</option>
                                </select>
                            </div>
                        </div>
                    </form>    
...
function __InformacionDataTable(){
    self.informacion_tabla = [ 
                               {'data' :'descripcion'    ,"name":"descripcion"     ,"title" : $.i18n.prop("caja.descripcion") ,"autoWidth" :true },
                               {'data' :'terminal'       ,"name":"terminal"        ,"title" : $.i18n.prop("caja.terminal")    ,"autoWidth" :true },
                               {'data' : 'created_at'        ,"name":"created_at"  ,"title" : $.i18n.prop("caja.created_at")  ,"autoWidth" :false,
                                    "render":function(created_at,type, row){
                                        return __displayDate_detail(created_at);
                                    }
                                },
                                {'data' : 'updated_at'        ,"name":"updated_at" ,"title" : $.i18n.prop("caja.updated_at")  ,"autoWidth" :false,
                                    "render":function(updated_at,type, row){
                                        return __displayDate_detail(updated_at);
                                    }
                                },
                               {'data' : 'usuario'       ,"name":"usuario"         ,"title" : $.i18n.prop("caja.usuario")     ,"autoWidth" :false,
                                    "render":function(usuario,type, row){
                                        return usuario.nombreUsuario;
                                    }
                               },
                               {'data' : 'estado'        ,"name":"estado"          ,"title" : $.i18n.prop("caja.estado")      ,"autoWidth" :false},
